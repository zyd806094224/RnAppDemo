...
export default class MarqueeHorizontal extends Component {
  static defaultProps = {
    ...
    width: Dimensions.get("window").width, //必须指定宽度 //375
    height: 50, //必须指定高度
    autoPlay: false, //是否自动播放
    duration: 10000, //执行完动画的总时间（不常用）
    speed: 0, //执行动画的速度
    delay: 0, //首次加载延时执行动画的时间
    useNativeDriver: false, //是否启动原生驱动
  ...
  constructor(props) {
    ...
    this.state = {
      list: [], //源数据
      animation: null, //动画对象
      viewWidth: props.contentLength,//deviceWidth*this.props.size*2, //文本数据实际宽度
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (this.props.type == MarqueeType.Simple) {
      if (
        this.state.viewWidth != prevState.viewWidth &&
  ...
  init() {
    ...
    // console.log('init', this.state.viewWidth + '--' + this.props.useNativeDriver);
    this.setState({
  ...
  animationTextView() {
    ...
    console.log('itemView---' + itemView.length)
    return (
      <Animated.View
        style={{
          flexDirection: 'row',
  ...
  layout() {
    ...
    return (
      <View
        ...
        <Text
          style={[styles.textStyle, {lineHeight: this.props.height}, textStyle]}
